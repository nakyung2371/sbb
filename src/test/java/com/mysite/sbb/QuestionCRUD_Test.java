...
public class QuestionCRUD_Test {
	...
	//Question 테이블을 CRUD하는 레파지토리를 객체 주입: @AutoWried	<- Test
	//QuestionRepository questionRepository = new QuestionRepository();		<- 개발자가 객체 생성
	...
	//Update 테스트: 기존의 레코드를 가지고 오고 setter를 통해서 수정 후 저장
	//@Test
	void jpaUpdate() {
		Optional<Question> op = 
				questionRepository.findById(152);
